...
export function CabinetPage() {
  ...
  const { activeList, sortBy, setSortBy, filterStatus, setFilterStatus, searchQuery, setSearchQuery, getFilteredGames } = useCabinetStore()
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
      ...
      <div style={{ padding: '24px 32px', flex: 1 }}>
        ...
        <div style={{ display: 'flex', gap: 12, marginBottom: 20, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: 4 }}>
            {(['all', 'unplayed', 'in_progress', 'completed', 'hundred_percent'] as const).map(s => (
              <button
                ...
                style={{ padding: '7px 12px', borderRadius: 7, border: 'none', background: filterStatus === s ? 'var(--surface2)' : 'transparent', color: filterStatus === s ? 'var(--text)' : 'var(--muted)', fontSize: 12, fontWeight: filterStatus === s ? 600 : 400, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif', outline: filterStatus === s ? '1px solid var(--border)' : 'none' }}
              >
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <select
              ...
              style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, color: 'var(--text)', fontSize: 12, padding: '7px 10px', outline: 'none', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}
            >
            ...
            <div style={{ display: 'flex', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, overflow: 'hidden' }}>
              {(['grid', 'list'] as ViewMode[]).map(mode => (
                <button
                  ...
                  style={{ padding: '7px 11px', border: 'none', background: viewMode === mode ? 'var(--surface2)' : 'transparent', color: viewMode === mode ? 'var(--text)' : 'var(--muted)', cursor: 'pointer', fontSize: 15, transition: 'all 0.15s', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                >
                  {mode === 'grid' ? (
                    ...
                    <svg width="15" height="15" viewBox="0 0 15 15" fill="currentColor">
                      ...
                      <rect x="0" y="6.25" width="15" height="2.5" rx="1" />
        ...
        {/* List view */}
        {games.length > 0 && viewMode === 'list' && (
          <ListView games={games} />
        )}
      </div>

      {showSearch && <SearchModal onClose={() => setShowSearch(false)} defaultList={activeList} />}
    </div>
  )
}